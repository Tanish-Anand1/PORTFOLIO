...
const RudraSimulatorModal = ({ isOpen, onClose }) => {
  ...
  const handleCommand = (e) => {
    ...
    setHistory(prev => [...prev, `root@G7510:~# ${inputVal}`, ...response]);
    setInputVal('');
  };
    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/85 backdrop-blur-[6px]">
        <motion.div
          ...
          className="w-full max-w-[640px] rounded-xl overflow-hidden border border-emerald-500/30 bg-[#0a0e14] shadow-[0_0_35px_rgba(23,147,209,0.15)] relative font-mono text-emerald-400"
        >
          {/* Scanline & CRT Effect */}
          ...
          {/* Header */}
          <div className="flex items-center justify-between border-b border-emerald-500/20 px-4 py-3 bg-[#10161f] relative z-10">
          ...
          {/* Output log */}
          <div className="h-96 overflow-y-auto p-4 text-xs space-y-2 leading-relaxed custom-scrollbar relative z-10 text-left">
            {history.map((line, index) => (
              <div key={index} className="whitespace-pre-wrap">
                {line.startsWith('root@G7510') ? (
                  <span className="text-emerald-350 font-bold">{line}</span>
                ) : (
                  line
                )}
              </div>
          ...
          {/* Input field */}
          <form onSubmit={handleCommand} className="border-t border-emerald-500/20 px-4 py-3 bg-[#10161f] flex items-center gap-2 relative z-10">
            <span className="text-emerald-350 font-bold shrink-0">root@G7510:~#</span>
            <input
              type="text"
              value={inputVal}
              onChange={(e) => setInputVal(e.target.value)}
              className="flex-1 bg-transparent border-none outline-none text-emerald-350 text-xs font-mono caret-sky-400 placeholder-sky-900"
